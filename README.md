...
前缀集合来自新鲜参考与仍在保留期内的历史。相同前缀不保证安全，不同前缀也可能是正常 CDN 调度；E/F 是自动规则分级，不能视为独立验证了污染。没有可用参考、无可比较 IPv4、查询失败、参考采集中断时显示待判定，不按空集合定罪。非 A 查询仍可监测可用性与时延，但不套用 IPv4 前缀规则。

可信查询缓存默认上限 300 秒，同时受实际 TTL 限制。TTL 到期不会主动高频发包；同域名共享参考采集，常规采集最短间隔 30 秒，期间使用有效历史或待判定。准备首次评 E/F 时可额外刷新可信参考，同域名共享并设置 300 秒冷却；全部查询受全局并发上限约束。手动刷新要求新的实际探测，保留可信历史。
...
从旧版本升级时，旧算法产生的自动 F 仅保留原始证据，有效状态改为待判定，不继续影响新版本 E/F 评级；历史人工判定保留。新规则的 E/F 按所选统计区间汇总，F 优先于 E；其余按可用率 45%、查询成功率 35%、P95 时延 20% 评分得到 A–D（A ≥95，B ≥85，C ≥70，其余 D）。P95 ≤50ms 获得满额时延分，≥2000ms 为零，中间线性递减。少于 10 个样本或覆盖不足 30 分钟显示待评估。
...
- 支持按评级 A、B、C、D、E、F 排序，也支持反向排序；待评估始终置于末尾。
- 总览表格右上方“导出当前结果”导出当前时间范围、筛选与排序下的监测汇总，包含质量指标。服务器详情的“导出原始记录”提供逐次探测证据；使用指南中的原始记录入口可导出全部节点。
